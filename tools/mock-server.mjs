...
http.createServer((req, res) => {
  ...
  if (u.pathname === '/api/availability') { const name = u.searchParams.get('name'), version = u.searchParams.get('version'); const has = sample.catalog.some(e => e.model.name === name && e.model.version === version); return json(res, { name, version, retrievedAt: new Date().toISOString(), regionsChecked: sample.regions.filter(r => r.hostsFoundry).length, availableIn: has ? sample.regions.filter((r, i) => r.hostsFoundry && i % 3 !== 1).length : 0, regions: sample.regions.filter(r => r.hostsFoundry).map((r, i) => ({ name: r.name, displayName: r.displayName, geography: r.geography, status: has && i % 3 !== 1 ? 'available' : 'absent' })) }); }
  if (u.pathname === '/api/deployments') return json(res, deployments());
  if (u.pathname === '/world.js') { res.writeHead(200, { 'content-type': 'text/javascript' }); return res.end(fs.readFileSync(path.join(root, 'src', 'FoundryModelExplorer', 'wwwroot', 'world.js'))); }
